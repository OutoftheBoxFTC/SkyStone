...
public class BlueAutonomous extends BasicOpmode {
    ...
    public void setup() {
        ...
        StateMachineManager strafeBeforeMovingToSkystones = new StateMachineManager(statemachine) {
            ...
            @Override
            public void setup() {
                terminator = new RelativeOrientationTerminator(position, new Vector3(0, -2, 90), 2);
                driveState.put("drive", system.driveForward(new Vector3(0, -2, 90), 0.7));
        ...
        StateMachineManager turnToIntakeSkystone = new StateMachineManager(statemachine) {
            ...
            public void update(SensorData sensors, HardwareData hardware) {
                if(bleh){
                    firstSkystone.set(position.getA() - 9, -12, position.getC());
                    bleh = false;
                }
